...
class PZ_License_System {
    ...
    public function create_tables() {
        ...
        $sql1 = "CREATE TABLE IF NOT EXISTS $table_school (
            ...
            teacher_username varchar(60) DEFAULT NULL,
            teacher_password varchar(255) DEFAULT NULL,
            student_user_id bigint(20) DEFAULT NULL,
            student_username varchar(60) DEFAULT NULL,
    ...
    public function register_user_roles() {
        ...
        if (!get_role('pz_teacher')) {
            add_role('pz_teacher', 'Psychology Zone Teacher', array(
                'read' => true,
                'view_study_materials' => true,
                'access_restricted_content' => true
            ));
        }
        
        if (!get_role('pz_student')) {
            add_role('pz_student', 'Psychology Zone Student', array(
                'read' => true,
                'view_study_materials' => true,
                'access_restricted_content' => true
            ));
    ...
    public function activate_license_on_payment($order_id) {
        ...
        if ($package_type === 'school') {
            ...
            $teacher_account = $this->create_teacher_account(0, $school_name);
            $student_account = $this->create_student_account(0, $school_name);
            ...
            $license_format = array('%d', '%d', '%s', '%s', '%s', '%s', '%s');
            
            if ($teacher_account && !is_wp_error($teacher_account)) {
                $license_data['teacher_user_id'] = $teacher_account['user_id'];
                $license_data['teacher_username'] = $teacher_account['username'];
                $license_data['teacher_password'] = $teacher_account['password'];
                $license_format[] = '%d';
                $license_format[] = '%s';
                $license_format[] = '%s';
                error_log('PZ License: Teacher account created - Username: ' . $teacher_account['username']);
            }
            
            if ($student_account && !is_wp_error($student_account)) {
                $license_data['student_user_id'] = $student_account['user_id'];
                $license_data['student_username'] = $student_account['username'];
                $license_data['student_password'] = $student_account['password'];
                $license_format[] = '%d';
                $license_format[] = '%s';
                $license_format[] = '%s';
                error_log('PZ License: Student account created - Username: ' . $student_account['username']);
            }
            ...
            if ($inserted === false) {
                error_log('PZ License: FAILED to insert school license. Error: ' . $wpdb->last_error);
                return;
            }
            
            $license_id = $wpdb->insert_id;
            error_log('PZ License: School license created with ID: ' . $license_id);
            
            if ($teacher_account && !is_wp_error($teacher_account)) {
                update_user_meta($teacher_account['user_id'], 'school_license_id', $license_id);
                error_log('PZ License: Teacher user meta updated with license ID: ' . $license_id);
            }
            
            if ($student_account && !is_wp_error($student_account)) {
                update_user_meta($student_account['user_id'], 'school_license_id', $license_id);
                error_log('PZ License: Student user meta updated with license ID: ' . $license_id);
            }
    ...
    public function get_user_school_license($user_id = null) {
        ...
        // Check if user is a teacher or student under a school license
        $school_license_id = get_user_meta($user_id, 'school_license_id', true);
        if ($school_license_id) {
            $license = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}pz_school_licenses 
                WHERE id = %d AND status = 'active' AND end_date > NOW()
                LIMIT 1",
                $school_license_id
            ));
    ...
    /**
     * Create teacher account for school license
     */
    private function create_teacher_account($license_id, $school_name) {
        // Generate unique username
        $base_username = sanitize_user(strtolower(str_replace(' ', '', $school_name)) . '_teacher');
        ...
        $email = $username . '@psychologyzone.local';
        $email_counter = 1;
        while (email_exists($email)) {
            $email = $username . $email_counter . '@psychologyzone.local';
            $email_counter++;
        }
        ...
        wp_update_user(array(
            'ID' => $user_id,
            'display_name' => $school_name . ' - Teacher',
            'first_name' => 'Teacher',
            'last_name' => $school_name
        ));
        
        // Link to school license
        update_user_meta($user_id, 'school_license_id', $license_id);
        update_user_meta($user_id, 'has_school_license', true);
        update_user_meta($user_id, 'is_school_teacher', true);
    ...
    /**
     * Create student account for school license
     */
    private function create_student_account($license_id, $school_name) {
        // Generate unique username
        $base_username = sanitize_user(strtolower(str_replace(' ', '', $school_name)) . '_student');
        $username = $base_username;
        $counter = 1;
        ...
        wp_update_user(array(
            'ID' => $user_id,
            'display_name' => $school_name . ' - Student',
            'first_name' => 'Student',
            'last_name' => $school_name
        ));
        ...
        update_user_meta($user_id, 'school_license_id', $license_id);
        update_user_meta($user_id, 'has_school_license', true);
        update_user_meta($user_id, 'is_school_student', true);
